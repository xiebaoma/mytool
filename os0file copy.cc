...
/** include fd concurrent hash set */
#include "fd0set.h"
...
#include "remote/remote0remote.h"
#include "role0role.h"

#define SPDB_SDK_NS spdb::sdk::file::
...
mysql_pfs_key_t innodb_dump_file_key;
#endif /* UNIV_PFS_IO */
...
static int os_file_lock(int fd, const char *name) {
  bool on_local = spdb_sdk::is_local_io(fd);

  if (likely(!on_local)) {
    /** sdk 不支持文件锁，默认返回0
    ib::info(ER_IB_MSG_749)   << "Unable to lock " << name << " error: " << errno;
    **/
...
ssize_t SyncFileIO::execute(const IORequest &request) {
  ...
  /** is meta fd and open the @parm (innodb_meta_on_local) */
  bool is_meta = spdb_sdk::is_local_io(m_fh);
  if (likely(!is_meta)) { // more io read on user space idb
    if (request.is_read()) {
      n_bytes = SPDB_SDK_NS pread(m_fh, m_buf, m_n, m_offset);
    } else {
      ut_ad(request.is_write());
      n_bytes = SPDB_SDK_NS pwrite(m_fh, m_buf, m_n, m_offset);
    }
    return (n_bytes);
  }
...
static std::string os_file_find_path_for_fd(os_file_t fd) {
  ...
  if (!on_local)
  {
    std::string path = spdb::sdk::file::get_path_use_fd(fd);
    if (path.empty()) {
      ib::info(ER_IB_MSG_1358) << " SPDB not support find path for fd " << fd;
      return "";
    }
    return path;
  }
...
static dberr_t os_file_punch_hole_posix(os_file_t fh, os_offset_t off,
                                        os_offset_t len) {
  bool on_local = spdb_sdk::is_local_io(fh);
...
static ulint os_file_get_last_error_low(bool report_all_errors,
                                        bool on_error_silent) {
  bool on_local = spdb_sdk::is_local_io();
  int err = 0;

  if (likely(!on_local)) {
    err = spdb::sdk::file::sp_errno;
  } else {
    err = errno;
  }
...
static int os_file_fsync_posix(os_file_t file) {
  bool on_local = spdb_sdk::is_local_io(file);
  if (likely(!on_local)) {
    return 0;
  }
...
static void os_parent_dir_fsync_posix(const char *path) {
  bool on_local = spdb_sdk::is_local_io(path);
  if (likely(!on_local)) {
    // SPDB with return empty
    return;
  }
...
static bool os_file_status_posix_mvcc(const char *path, bool *exists,
                                 os_file_type_t *type) {
  ...
  bool on_local = spdb_sdk::is_local_io(path);
  if (likely(!on_local)) {
    ret = SPDB_SDK_NS file_exist_rw_version(path);
  } else {
...
static bool os_file_status_posix(const char *path, bool *exists,
  ...
  bool on_local = spdb_sdk::is_local_io(path);
  if (likely(!on_local)) {
    ret = SPDB_SDK_NS stat(path, &statinfo);
  } else {
    ret = stat(path, &statinfo);
  }
...
bool os_file_create_directory(const char *pathname, bool fail_if_exists) {
  bool on_local = spdb_sdk::is_local_io(pathname);

  int rcode = 0, err = 0;
  if (likely(!on_local)) {
    rcode = SPDB_SDK_NS mkdir(pathname, 0770);
  } else {
  ...
  if (!(rcode == 0 || (err == EEXIST && !fail_if_exists))) {
    /* failure */
...
bool os_file_scan_directory(const char *path, os_dir_cbk_t scan_cbk,
  ...
  bool on_local = spdb_sdk::is_local_io(path);
  if (likely(!on_local)) {
    SPDB_SDK_NS DIR *directory = nullptr;
    SPDB_SDK_NS dirent *entry = nullptr;
    directory = SPDB_SDK_NS opendir(path);
    if (directory == nullptr) {
      os_file_handle_error_no_exit(path, "opendir", false);
      return (false);
    }

    entry = SPDB_SDK_NS readdir(directory);

    while (entry != nullptr) {
      scan_cbk(path, entry->d_name.c_str());
      entry = SPDB_SDK_NS readdir(directory);
    }

    SPDB_SDK_NS closedir(directory);

    if (is_drop) {
      int err;
      err = SPDB_SDK_NS rmdir(path);
...
pfs_os_file_t os_file_create_func(const char *name, ulint create_mode,
  ...
  *success = false;
  ...
  if (create_mode == OS_FILE_OPEN || create_mode == OS_FILE_OPEN_RAW ||
    ...
    space_id_create = file_create_space_id != SPACE_UNKNOWN;
  ...
  bool on_local = spdb_sdk::is_local_io(name);
  do {

    if (likely(!on_local)) {
      DBUG_PRINT("sdk",("os_file_create_func create %s, use space id %d", name, space_id_create));
      if (space_id_create) {
        file.m_file = SPDB_SDK_NS create_with_space_id(name, file_create_space_id);
        file_create_space_id = SPACE_UNKNOWN; //reset
      } else {
        file.m_file = SPDB_SDK_NS open(name, create_flag, os_innodb_umask);
	  }
    } else {
      file.m_file = open(name, create_flag, os_innodb_umask);
      if (file.m_file >= LOCAL_FD_MAX) {
          DBUG_PRINT("sdk",("os_file_create_func file fd error %d", file.m_file));
          close(file.m_file);
          errno = ENFILE;
          file.m_file = -1;
      }
...
pfs_os_file_t os_file_create_simple_no_error_handling_func(
  ...
  *success = false;
  ...
  bool on_local = spdb_sdk::is_local_io(name);
  if (likely(!on_local)) {
    file.m_file = SPDB_SDK_NS open(name, create_flag, umask);
  } else {
    file.m_file = open(name, create_flag, umask);
    if (file.m_file >= LOCAL_FD_MAX) {
        DBUG_PRINT("sdk",("os_file_create_simple_no_error_handling_func file fd error %d", file.m_file));
        close(file.m_file);
        errno = ENFILE;
        file.m_file = -1;
    }
//    spdb_sdk::add_meta_fd_on_open(file.m_file);
  }
...
bool os_file_delete_if_exists_func(const char *name, bool *exist) {
  ...
  if (rst != 0) {// couldn't unlink
    if (err == ENOENT) {  // that's because file was missing, not an error
      if (exist != nullptr) {
        *exist = false;
      }
      return true;
...
bool os_file_rename_func(const char *oldpath, const char *newpath) {
  ...
  bool on_local = spdb_sdk::is_local_io(oldpath);
  int ret = 0;

  if (likely(!on_local)) {
    ret = SPDB_SDK_NS rename(oldpath, newpath);
  } else {
...
bool os_file_close_func(os_file_t file) {
  ...
  int ret = 0;
  if (likely(!on_local)) {
    ret = SPDB_SDK_NS close(file);
  } else {
    ret = close(file);
    spdb_sdk::remove_meta_fd_on_close(file);
  }
  DBUG_PRINT("sdk", ("on_local %d, file fd %d", int(on_local), file));
...
bool os_file_advise(pfs_os_file_t file, os_offset_t offset, os_offset_t len,
                    ulint advice) {
  bool on_local = spdb_sdk::is_local_io(file.m_file);
  if (likely(!on_local)) {
    ib::info(ER_IB_MSG_749)
        << "error: SPDB not support os_file advise ";
    return true;
  }
...
os_offset_t os_file_get_size(pfs_os_file_t file) {
  bool on_local = spdb_sdk::is_local_io(file.m_file);
  if (likely(!on_local)) {
      os_offset_t file_size = SPDB_SDK_NS file_size(file.m_file);
      return (file_size);
  }
...
os_file_size_t os_file_get_size(const char *filename) {
  ...
  int ret = 0;
  bool on_local = spdb_sdk::is_local_io(filename);
  if (likely(!on_local)) {
    ret = SPDB_SDK_NS stat(filename, &s);
  } else {
    ret = stat(filename, &s);
  }
...
static dberr_t os_get_free_space_posix(const char *path, uint64_t &free_space) {
  bool on_local = spdb_sdk::is_local_io(path);
  if (likely(!on_local)){
    ulong res = spdb::sdk::file::get_free_space();
    if (res == 0) {
      ib::error(ER_IB_MSG_749) << "error: SPDB sdk not have free space";
      return (DB_FAIL);
    }
    free_space = res;
  }
...
static dberr_t os_file_get_status_posix(const char *path,
  ...
  if (check_rw_perm && (stat_info->type == OS_FILE_TYPE_FILE ||
    ...
    int fh = 0;
    if (likely(!on_local)) {
      fh = SPDB_SDK_NS open(path, access, os_innodb_umask);
    } else {
      fh = open(path, access, os_innodb_umask);
      if (fh >= LOCAL_FD_MAX) {
          DBUG_PRINT("sdk", ("os_file_create_simple_no_error_handling_func file fd error %d", fh));
          close(fh);
          errno = ENFILE;
          fh = -1;
      }
//      spdb_sdk::add_meta_fd_on_open(fh);
    }
    ...
    if (fh == -1) {
      ...
      if (likely(!on_local)) {
        SPDB_SDK_NS close(fh);
      } else {
        close(fh);
        spdb_sdk::remove_meta_fd_on_close(fh);
      }
    }
  }
...
static bool os_file_truncate_posix(const char *pathname, pfs_os_file_t file,
  ...
  if (likely(!on_local)) {
    ib::error(ER_IB_MSG_749)
        << "error: SPDB not support os_file_truncate_posix ";
    return (-1);
  }
...
bool os_file_close_no_error_handling_func(os_file_t file) {
  bool on_local = spdb_sdk::is_local_io(file);
  if (likely(!on_local)) {
    return (SPDB_SDK_NS close(file) != -1);
  }
  spdb_sdk::remove_meta_fd_on_close(file);
  return (close(file) != -1);
}
...
void Dir_Walker::walk_posix(const Path &basedir, bool recursive, Function &&f) {
  ...
  while (!directories.empty()) {
    ...
    bool on_local = spdb_sdk::is_local_io(current.m_path.c_str());
    if (likely(!on_local)) {
      SPDB_SDK_NS DIR *parent = SPDB_SDK_NS opendir(current.m_path.c_str());
      ...
      struct SPDB_SDK_NS dirent *dirent = nullptr;

      for (;;) {
        dirent = SPDB_SDK_NS readdir(parent);

        if (dirent == nullptr) {
          break;
        }

        if (strcmp(dirent->d_name.c_str(), ".") == 0 ||
            strcmp(dirent->d_name.c_str(), "..") == 0) {
          continue;
        }
...
bool os_file_set_nocache(int fd [[maybe_unused]],
  ...
  if (likely(!on_local)) {
    ib::info(ER_IB_MSG_749)
        << "error: SPDB not support os_file_set_nocache";
    return  true;
  }
...
bool os_file_set_size_fast(const char *name, pfs_os_file_t pfs_file,
  ...
  int ret = 0, err = 0;
  bool on_local = spdb_sdk::is_local_io(pfs_file.m_file);
  if (likely(!on_local)) {
    ret =
        SPDB_SDK_NS fallocate(pfs_file.m_file, FALLOC_FL_ZERO_RANGE, offset, size - offset);
  } else {
    ret =
  ...
  if (print_message) {
    ...
    ib::info(ER_IB_MSG_1359) << "fallocate() failed with errno " << err
...
bool os_file_set_size(const char *name, pfs_os_file_t file, os_offset_t offset,
  ...
  // hengwei.chen: 如果使用SPDB设定文件大小，至少要满足一个block的大小，即4MB
  buf_size = 4ul * 1024 * 1024;
...
bool os_file_seek(const char *pathname, os_file_t file, os_offset_t offset) {
  ...
  bool on_local = spdb_sdk::is_local_io(file);
  if (likely(!on_local)) {
    ret = SPDB_SDK_NS lseek(file, offset, SEEK_SET);
  } else {
...
dberr_t os_file_copy_func(os_file_t src_file, os_offset_t src_offset,
                          ...
                          uint size) {
  // bool on_local = spdb_sdk::is_local_io(src_file);
  // if (likely(!on_local)) {
    // ib::info(ER_IB_MSG_827) << "SPDB sdk not support os_file_copy_func.";
    // return (DB_IO_ERROR);
  // }
  dberr_t err;
...
#else  /* !_WIN32 */
  if (is_source_role() && likely(!spdb_sdk::is_local_io(path))) {
    std::string_view path_view(path);
    if (path_view.size() >= 4 &&
      path_view.compare(path_view.size() - 4, 4, ".ibd") == 0) {
      return (os_file_status_posix_mvcc(path, exists, type));
    }
  }
...
bool os_is_sparse_file_supported(pfs_os_file_t fh) {
  bool on_local = spdb_sdk::is_local_io(fh.m_file);
  if (likely(!on_local)) {
    return false;
  }
...
dberr_t os_get_free_space(const char *path, uint64_t &free_space) {
  ...
  if (likely(!on_local)) {
    ib::error(ER_IB_MSG_749)
        << "error: SPDB not support os_get_free_space ";
    return (DB_FAIL);
  }
...
dberr_t os_aio_func(IORequest &type, AIO_mode aio_mode, const char *name,
  ...
  if (aio_mode == AIO_mode::SYNC) {
    ...
    if (type.is_read()) {
      if (type.is_page_server()) {
        DBUG_PRINT("pg", ("aio sync --> pg read space id %u, page no %u", type.get_page_id().space(),  type.get_page_id().page_no()));
        return page_server::page::read_page(type.get_page_id(), buf);
      }
    ...
    ut_ad(type.is_write());
    if (type.is_page_server()) {
      DBUG_PRINT("pg", ("aio sync --> pg flush space id %u, page no %u", type.get_page_id().space(),  type.get_page_id().page_no()));
      return DB_SUCCESS;
    }
...
class SimulatedAIOHandler {
 ...
 private:
  ...
  void read(Slot *slot) {
    if (slot->type.is_page_server()) {
      DBUG_PRINT("pg", ("aio async --> pg read space id %u, page no %u", slot->type.get_page_id().space(),  slot->type.get_page_id().page_no()));
      dberr_t err = page_server::page::read_page(slot->type.get_page_id(), slot->ptr);
      ut_a(err == DB_SUCCESS);
      return;
    }
    DBUG_PRINT("pg", ("aio async --> normal read file name %s, offset %lu", slot->name, slot->offset));
    dberr_t err =
  ...
  void write(Slot *slot) {
    dberr_t err = DB_SUCCESS;
    if (!slot->type.is_page_server()) {
       err = os_file_write_func(slot->type, slot->name, slot->file.m_file,
                                     slot->ptr, slot->offset, slot->len);
      DBUG_PRINT("pg", ("aio async --> normal flush file name %s, offset %lu", slot->name, slot->offset));
    }
#ifdef UNIV_DEBUG
    else {
      DBUG_PRINT("pg", ("aio async --> pg flush space id %u, page no %u", slot->type.get_page_id().space(),  slot->type.get_page_id().page_no()));
    }
 #endif
